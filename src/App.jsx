...
export default function App() {
  ...
  const getRandomTopics = (k) => {
    ...
    for (let i = 0; i < k; i++) res += ', ' + topics[getRandomInt(topics.length)];
    return res;
  };
  ...
  useEffect(() => {
    if (!videoId) return;
    if (videoIsLoading || !videoData) return;
  ...
  useEffect(() => {
    ...
    if (videoId && videoIsLoading) return;
  ...
  const addButton = (
    videoId && videoData && (
      <GridItem rows={1} cols={1} size={SIZE}>
        <GridButton
          onClick={async () => {
            try {
              const response = await axios.post('https://harbour.dev.is/api/playlists', {
